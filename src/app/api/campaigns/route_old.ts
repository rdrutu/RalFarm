...
export const GET = withAuth(async (req) => {
  ...
  try {
    ...
    if (year) query = query.eq('crop_year', parseInt(year))
    ...
    // Calculează statisticile pentru fiecare campanie
    const campaignsWithStats = data?.map((campaign: any) => {
      const expenses = campaign.expenses || []
      const sales = campaign.stock_movements?.filter((sm: any) => sm.movement_type === 'sale') || []
      
      const totalCosts = expenses.reduce((sum: number, exp: any) => sum + (exp.amount_ron || 0), 0)
      const totalRevenue = sales.reduce((sum: number, sale: any) => sum + (sale.quantity_kg * sale.unit_price_ron || 0), 0)
